...
// 三步流水线（Vercel 60s超时，可在后端串行完成）
async function rewriteOnce(apiKey, text) {
  const origLen = text.replace(/\s/g, '').length;
  const { model, maxTokens } = pickModel(origLen);
  const params = { temperature: 1.0, top_p: 0.9, presence_penalty: 0.6, frequency_penalty: 0.4 };

  // 第一步：打散句子
  const step1 = await callKimi(apiKey, PROMPTS.step1,
    `请处理以下文本：\n\n${text}`, model, maxTokens, params);

  // 第二步：降AI率改写
  const step2 = await callKimi(apiKey, PROMPTS.step2,
    `请改写以下文本：\n\n${step1}`, model, maxTokens, params);

  // 第三步：同义词替换
  const step3 = await callKimi(apiKey, PROMPTS.step3(origLen),
    `请对以下文本进行同义词替换：\n\n${step2}`, model, maxTokens, params);
...
exports.handler = async (event) => {
  ...
  try {
    // 超过5000字使用分段改写，避免截断
    const result = await rewriteOnce(apiKey, text);
